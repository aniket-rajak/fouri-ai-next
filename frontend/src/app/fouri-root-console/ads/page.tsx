...
export default function OwnerAdsPage() {
  ...
  const [form, setForm] = useState({ title: "", description: "", imageUrl: "", ctaText: "Learn More", ctaLink: "" });
  ...
  const resetForm = () => {
    setForm({ title: "", description: "", imageUrl: "", ctaText: "Learn More", ctaLink: "" });
    setEditing(null);
    setShowForm(false);
  };

  const handleEdit = (ad: Ad) => {
    setForm({ title: ad.title, description: ad.description || "", imageUrl: ad.imageUrl, ctaText: ad.ctaText, ctaLink: ad.ctaLink });
    setEditing(ad);
    setShowForm(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await api(`/ads/${editing.id}`, { method: "PUT", body: JSON.stringify(form) });
      } else {
